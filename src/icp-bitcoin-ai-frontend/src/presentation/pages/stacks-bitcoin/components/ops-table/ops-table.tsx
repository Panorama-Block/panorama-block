...
import styles from './ops-table-styles.module.scss'
...
const OpsTable: React.FC<Props> = ({ title, data }: Props) => {
        ...
        <Table >
          <TableHeader>
            <TableRow className="border-zinc-600 hover:bg-[#f2f2f210]">
              ...
              <TableHead className="text-white font-bold text-md text-nowrap">Input1 Address</TableHead>
              ...
              <TableHead className="text-white font-bold text-md text-nowrap">Transaction Value</TableHead>
              ...
              <TableHead className="text-white font-bold text-md text-nowrap">Script Hex</TableHead>
          ...
          <TableBody>
            {
              data.slice((actual - 1) * perPage, ((actual - 1) * perPage) + perPage).map((row, index) => {
                return <TableRow key={`ops-${index}`} className="border-zinc-700 text-[#A0AEC0] hover:bg-[#f2f2f210]">
                  ...
                  <TableCell className="font-medium underline text-blue-400" dangerouslySetInnerHTML={{ __html: row.input1_address }}></TableCell>
                  ...
                  <TableCell className="text-zinc-200 font-medium">{row.tx_value}</TableCell>
                  ...
                  <TableCell className="text-zinc-200 font-medium">{row.script_hex}</TableCell>
                </TableRow>
              })
            }
          </TableBody>
